{ config, lib, pkgs, ... }: {
  config.assertions = lib.flatten
    (lib.flip lib.mapAttrsToList config.services.github-runners (name: cfg:
      map (lib.mkIf cfg.enable) [
        {
          assertion = !cfg.noDefaultLabels || (cfg.extraLabels != [ ]);
          message =
            "`services.github-runners.${name}`: The `extraLabels` option is mandatory if `noDefaultLabels` is set";
        }
        {
          assertion = cfg.group == null || cfg.user != null;
          message = ''
            `services.github-runners.${name}`: Setting `group` while leaving `user` unset runs the service as `root`. If this is really what you want, set `user = "root"` explicitly'';
        }
      ]));

  config.systemd.services = let
    enabledRunners =
      lib.filterAttrs (_: cfg: cfg.enable) config.services.github-runners;
  in (lib.flip lib.mapAttrs' enabledRunners (name: cfg:
    let
      ...
      package = cfg.package.override (old:
        lib.optionalAttrs (lib.hasAttr "nodeRuntimes" old) {
    ...
    in lib.nameValuePair svcName {
      ...
      path = (with pkgs; [ bashInteractive coreutils git gnutar gzip ])
      ...
      serviceConfig = lib.mkMerge [
        {
          ...
          ExecStartPre = let
            ...
            runnerCredFiles =
              [ ".credentials" ".credentials_rsaparams" ".runner" ];
              ...
              ''
                if [[ -e "${newConfigTokenPath}" ]]; then
                  ...
                  args=(
                    ...
                    --labels ${
                      lib.escapeShellArg
                      (lib.concatStringsSep "," cfg.extraLabels)
                    }
                    ${
                      lib.optionalString (cfg.name != null)
                    ...
                    ${
                      lib.optionalString cfg.noDefaultLabels
                      "--no-default-labels"
                    }
                  )
            ...
            '';
          in map
          (x: "${x} ${lib.escapeShellArgs [ stateDir workDir logsDir ]}") [
